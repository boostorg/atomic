...
private:
    ...
    wait_until_fallback(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, typename Clock::time_point now)
    ...
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_abs_timeout(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout)
        noexcept(noexcept(Clock::now()))
    {
        ...
        while (res.value == old_val)
        {
            ...
            if (err < 0)
            {
                ...
                if (BOOST_UNLIKELY(err == ENOSYS))
                    return wait_until_fallback< Clock >(storage, old_val, order, timeout, Clock::now());
            }
    ...
    template< typename Clock >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_dispatch(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, futex_wait_fallback)
        noexcept(noexcept(Clock::now()))
    {
        return wait_until_fallback< Clock >(storage, old_val, order, timeout, Clock::now());
    }
    ...
    template< typename Clock, clockid_t ClockId >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_dispatch(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, futex_wait_clock< ClockId >)
        noexcept(noexcept(Clock::now()))
    {
        return wait_until_abs_timeout< Clock, ClockId >(storage, old_val, order, timeout);
    }

public:
    template< typename Clock, typename Duration >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until(storage_type const volatile& storage, storage_type old_val, memory_order order, std::chrono::time_point< Clock, Duration > timeout)
        noexcept(noexcept(wait_until_dispatch< Clock >(storage, old_val, order, timeout, typename select_futex_wait< Clock >::type())))
    {
        return wait_until_dispatch< Clock >(storage, old_val, order, timeout, typename select_futex_wait< Clock >::type());
    }
...
public:
    ...
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until(storage_type const volatile& storage, storage_type old_val, memory_order order, std::chrono::time_point< Clock, Duration > timeout)
        noexcept(noexcept(wait_until_fallback< Clock >(storage, old_val, order, timeout, Clock::now())))
    {
        return wait_until_fallback< Clock >(storage, old_val, order, timeout, Clock::now());
    }
    ...
    wait_for(storage_type const volatile& storage, storage_type old_val, memory_order order, std::chrono::duration< Rep, Period > timeout) noexcept
    {
        ...
        if (BOOST_LIKELY(timeout.count() >= 0))
        {
            ...
            {
                const int64_t nsec = static_cast< int64_t >(now.tv_nsec) + atomics::detail::chrono::ceil< std::chrono::nanoseconds >(timeout).count();
                const int64_t sec = static_cast< int64_t >(now.tv_sec) + nsec / 1000000000;
    ...
    use_wait_until_fallback:
        ...
        return wait_until_fallback< std::chrono::steady_clock >(storage, old_val, order, now + timeout, now);
    }
