...
#if !defined(BOOST_WINDOWS)
    void wait_until(clockid_t clock_id, std::chrono::nanoseconds time_since_epoch) noexcept
    {
        lock_pool::wait_until(this->get_lock_state(), m_wait_state, clock_id, time_since_epoch);
    }
#endif // !defined(BOOST_WINDOWS)

    void wait_for(std::chrono::nanoseconds rel_timeout) noexcept
    {
        lock_pool::wait_for(this->get_lock_state(), m_wait_state, rel_timeout);
    }
};
