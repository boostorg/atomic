...
{
    template< typename T, clockid_t = posix_clock_traits< Clock >::clock_id >
    static std::true_type check_posix_clock_traits_clock_id(T);
    static std::false_type check_posix_clock_traits_clock_id(...);

    using type = decltype(has_posix_clock_traits_impl< Clock >::check_posix_clock_traits_clock_id(0));
};
