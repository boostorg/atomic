...
private:
    ...
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_abs_timeout(storage_type const volatile& storage, storage_type old_val, memory_order order, _umtx_time const& umt) noexcept
    {
        wait_result< storage_type > res{ base_type::load(storage, order), false };
        while (res.value == old_val)
        {
    ...
    template< typename Clock >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_dispatch(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, std::false_type)
        noexcept(noexcept(Clock::now()))
    {
    ...
    template< typename Clock >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_dispatch(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, std::true_type)
        noexcept
    {
        _umtx_time umt{};
        ...
        return wait_until_abs_timeout(storage, old_val, order, umt);
    }
...
public:
    ...
    wait_until(storage_type const volatile& storage, storage_type old_val, memory_order order, std::chrono::time_point< Clock, Duration > timeout)
        noexcept(noexcept(wait_until_dispatch< Clock >(storage, old_val, order, timeout, std::integral_constant< bool, has_posix_clock_traits< Clock >::value >())))
    {
        return wait_until_dispatch< Clock >(storage, old_val, order, timeout, std::integral_constant< bool, has_posix_clock_traits< Clock >::value >());
    }
...
private:
    template< typename Clock >
    static BOOST_FORCEINLINE wait_result< storage_type >
    wait_until_impl(storage_type const volatile& storage, storage_type old_val, memory_order order, typename Clock::time_point timeout, typename Clock::time_point now)
        noexcept(noexcept(Clock::now()))
    {
        timespec ts{};
        wait_result< storage_type > res{ base_type::load(storage, order), false };
