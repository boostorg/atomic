    ...
    public wait_state_base
{
    ...
    bool wait_until(lock_state& state, clockid_t clock_id, std::chrono::nanoseconds time_since_epoch) noexcept;
    //! Blocks in the wait operation until notified or timeout
...
//! Blocks in the wait operation until notified or timeout
inline bool wait_state::wait_until(lock_state& state, clockid_t clock_id, std::chrono::nanoseconds time_since_epoch) noexcept
{
    ...
    {
        timespec ts{};
        std::chrono::nanoseconds::rep nsec = time_since_epoch.count();
        std::chrono::nanoseconds::rep sec = nsec / 1000000000;
        ...
        return pthread_cond_clockwait(&m_cond, &state.m_mutex, clock_id, &ts) == ETIMEDOUT;
    }
    ...
    std::chrono::nanoseconds rel_timeout = time_since_epoch - (std::chrono::seconds(ts.tv_sec) + std::chrono::nanoseconds(ts.tv_nsec));
    if (BOOST_UNLIKELY(rel_timeout <= std::chrono::nanoseconds::zero()))
        return true;

    return wait_for(state, rel_timeout);
}
...
inline bool wait_state::wait_until(lock_state& state, clockid_t clock_id, std::chrono::nanoseconds time_since_epoch) noexcept
{
...

    {
        ...
        if (BOOST_LIKELY(clock_gettime(clock_id, &now) == 0))
        {
            ...
            do
            {
                std::chrono::nanoseconds::rep nsec = (time_since_epoch - (std::chrono::seconds(now.tv_sec) + std::chrono::nanoseconds(now.tv_nsec))).count();
                if (nsec <= 0)
                {
                    timed_out = true;
                    goto done; // to avoid "unused label" warnings
                }
...
inline bool wait_state::wait_for(lock_state& state, std::chrono::nanoseconds rel_timeout) noexcept
{
    ...
    return wait_until(state, clock_id, std::chrono::seconds(now.tv_sec) + std::chrono::nanoseconds(now.tv_nsec) + rel_timeout);
}
...
inline bool wait_state::wait_for(lock_state& state, std::chrono::nanoseconds rel_timeout) noexcept
{
    ...
    return !boost::winapi::SleepConditionVariableSRW(&m_cond, &state.m_mutex, tout, 0u) && boost::winapi::GetLastError() == boost::winapi::ERROR_TIMEOUT_;
}
...
BOOST_ATOMIC_DECL bool wait_until(void* vls, void* vws, clockid_t clock_id, std::chrono::nanoseconds time_since_epoch) noexcept
{
    BOOST_ASSERT(vls != nullptr);
    ...
    if (BOOST_LIKELY(ws != nullptr))
    {
        return ws->wait_until(*ls, clock_id, time_since_epoch);
    }
    else
    {
        // A conforming wait operation must unlock and lock the mutex to allow a notify to complete
        ...
        return clock_gettime(clock_id, &ts) == 0 &&
            (time_since_epoch - (std::chrono::seconds(ts.tv_sec) + std::chrono::nanoseconds(ts.tv_nsec))).count() <= 0;
    }
}
